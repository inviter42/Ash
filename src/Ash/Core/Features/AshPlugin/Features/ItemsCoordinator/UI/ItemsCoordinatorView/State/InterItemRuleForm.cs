...
using Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.Components.InterItemCoordination;
using Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.Types;
using Character;
using OneOf;
using InputsValues = OneOf.OneOf<
    Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.Types.ItemWearFormData,
    Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.Types.ItemAccessoryFormData,
    Character.WEAR_SHOW,
    bool,
    Female,
    Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.Types.GenerateReverseRulesEnum.GenerateReverseRules
>;

namespace Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.State
{
    internal class InterItemRuleForm
    {
        ...
        private bool ValidateInputs() {
            if (!FormData.ContainsKey(MasterItemSelectionComponent.MasterItemFormDataKey)) {
                ItemsCoordinator.Logger.LogWarning("MasterItem form data was not found. Rule will not be created.");
                FormData.Clear();
                return false;
            }

            if (!FormData.ContainsKey(InterItemRuleDetailsComponent.MasterItemStateFormDataKey)) {
                ItemsCoordinator.Logger.LogWarning("MasterItem state form data was not found. Rule will not be created.");
                FormData.Clear();
                return false;
            }

            if (!FormData.ContainsKey(InterItemRuleDetailsComponent.SlaveItemFormDataKey)) {
                ItemsCoordinator.Logger.LogWarning("SlaveItem form data was not found. Rule will not be created.");
                FormData.Clear();
                return false;
            }

            if (!FormData.ContainsKey(InterItemRuleDetailsComponent.SlaveItemStateFormDataKey)) {
                ItemsCoordinator.Logger.LogWarning("SlaveItem state form data was not found. Rule will not be created.");
                FormData.Clear();
                return false;
            }

            if (!FormData.ContainsKey(FemaleFormDataKey)) {
                ItemsCoordinator.Logger.LogWarning("Female form data was not found. Rule will not be created.");
                FormData.Clear();
                return false;
            }

            if (!FormData.ContainsKey(InterItemRuleDetailsComponent.GenerateReverseRulesFormDataKey)) {
                ItemsCoordinator.Logger.LogWarning(
            ...
            if (masterItemFormData.IsT0 && !masterItemStateFormData.IsT2
                ...
                ItemsCoordinator.Logger.LogWarning("Item and its state have non-matching types. Rule will not be created.");
                FormData.Clear();
                return false;
            }

            if (!femaleFormData.IsT4) {
                ItemsCoordinator.Logger.LogWarning("Female form data is of a wrong type. Rule will not be created.");
                FormData.Clear();
                return false;
            }
            ...
            if (ReferenceEquals(masterItemFormData.Value, null)) {
                ItemsCoordinator.Logger.LogWarning("Master item is null. Rule will not be created.");
                FormData.Clear();
                return false;
            }

            if (ReferenceEquals(slaveItemFormData.Value, null)) {
                ItemsCoordinator.Logger.LogWarning("Slave item is null. Rule will not be created.");
                FormData.Clear();
                return false;
            }

            if (ReferenceEquals(femaleFormData.AsT4, null)) {
                ItemsCoordinator.Logger.LogWarning("Female is null. Rule will not be created.");
                FormData.Clear();
                return false;
            }
