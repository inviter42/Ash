...
using System.Linq;
using Ash.Core.Features.AshPlugin.Common.Misc;
using Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.State;
...
using static Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.State.InterItemRuleForm;

namespace Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.UI.ItemsCoordinatorView.Components.InterItemCoordination
{
    internal static class InterItemRuleDetailsComponent
    {
        ...
        internal static void DrawInterItemRuleDetailsView(InterItemRuleForm form) {
            ...
            if (!formData.TryGetValue(FemaleFormDataKey, out var femaleFormData)) {
                ItemsCoordinator.Logger.LogWarning("Female is null.");
                ItemsCoordinator.Logger.LogWarning(Environment.StackTrace);
                formData.Clear();
                return;
            }
            ...
            if (!formData.TryGetValue(MasterItemSelectionComponent.MasterItemFormDataKey, out var masterItemFormDataRaw)) {
                ItemsCoordinator.Logger.LogWarning("MasterItem form data doesn't exist.");
                return;
            }
        ...
        private static void MasterItemStateSelection(InterItemRuleForm form, Female activeFemale, OneOf<ItemWearFormData, ItemAccessoryFormData> masterItemFormData) {
            switch (masterItemFormData.Value) {
                ...
                default:
                    ItemsCoordinator.Logger.LogError("Unknown type of MasterItem form data.");
                    return;
            }
        }
        ...
        private static void MasterItemAccessoryStateSelectionTitle(Female activeFemale, ItemAccessoryFormData masterItemFormDataAccessory) {
            if (masterItemFormDataAccessory.AccessoryParameter == null) {
                ItemsCoordinator.Logger.LogWarning("AccessoryParameter for MasterItem is null.");
                return;
            }
            ...
            if (accessoryDataForMasterItem == null) {
                ItemsCoordinator.Logger.LogWarning("AccessoryData for MasterItem is null.");
                return;
            }
        ...
        private static void SlaveItemStateSelection(InterItemRuleForm form) {
            ...
            switch (slaveItemFromData.Value) {
                ...
                default:
                    ItemsCoordinator.Logger.LogError("Unknown type of SlaveItem form data.");
                    return;
            }
        }
