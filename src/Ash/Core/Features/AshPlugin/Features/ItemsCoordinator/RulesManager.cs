...
namespace Ash.Core.Features.AshPlugin.Features.ItemsCoordinator
{
    internal static class RulesManager
    {
        ...
        internal static void AddRule(
            ...
            if (!female) {
                ItemsCoordinator.Logger.LogWarning("Attempting to add the Rule, but Female is null");
                return;
            }
            ...
            if (idx != -1) {
                ...
                if (!slavesHashSet.Add(slaveItem))
                    ItemsCoordinator.Logger.LogWarning("Failed to add new SlaveItem to HashSet - already exists?");
            }
            else {
            ...
            foreach (var f in females) {
                switch (masterItemFormData.Value) {
                    ...
                    default:
                        ItemsCoordinator.Logger.LogError("[RulesManager] Unknown type of MasterItem!");
                        return;
                }
            }
        ...
        internal static void RemoveRule(BaseItem masterItem, SlaveItem slaveItem) {
            ...
            var idx = InterItemRuleSets.FindIndex(rule => rule.MasterItem == masterItem);
            if (idx == -1) {
                ItemsCoordinator.Logger.LogWarning("Unable to remove the rule for this MasterItem.");
                return;
            }

            if (!InterItemRuleSets[idx].SlaveItems.Remove(slaveItem)) {
                ItemsCoordinator.Logger.LogWarning("Unable to remove the rule - SlaveItem was not found in the HashSet<SlaveItems>");
                return;
            }
        ...
        internal static void RemoveRule(HPosRuleSet ruleset) {
            ...
            if (idx == -1) {
                ItemsCoordinator.Logger.LogWarning("Unable to remove the rule for this HPosItem.");
                return;
            }
        ...
        private static BaseItem MakeMasterItem(OneOf<ItemWearFormData, ItemAccessoryFormData> masterItemData) {
            switch (masterItemData.Value) {
                ...
                default:
                    ItemsCoordinator.Logger.LogError("Unknown type of ItemMasterData!");
                    return null;
            }
        }
        ...
        private static void GenerateReverseRules(
            ...
            switch (masterItemFormData.Value) {
                case ItemWearFormData masterCastToWear:
                {
                    ...
                    foreach (WEAR_SHOW otherShow in Enum.GetValues(typeof(WEAR_SHOW))) {
                        ...
                        switch (slaveItemFormData.Value) {
                            ...
                            default:
                                ItemsCoordinator.Logger.LogError("Unknown item type of SlaveItem");
                                return;
                        }
                    }
                ...
                default:
                    ItemsCoordinator.Logger.LogError("Unknown item type of MasterItem");
                    return;
            }
        }
    }
