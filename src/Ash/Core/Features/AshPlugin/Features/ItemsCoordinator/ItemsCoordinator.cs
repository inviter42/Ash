...
using System.Linq;
using Ash.Core.Features.AshPlugin.Features.ItemsCoordinator.Types;
using Ash.Logging;
using Ash.Utility.GlobalUtils;
using Character;
using OneOf;

namespace Ash.Core.Features.AshPlugin.Features.ItemsCoordinator
{
    internal static class ItemsCoordinator
    {
        internal static readonly AshLogger Logger = new AshLogger(LoggingSettings.LoggingModules.ItemsCoordinator);
        ...
        internal static void ApplyRules(Female female, List<InterItemRuleSet> ruleset) {
            if (SkipRulesApplication) {
                Logger.LogDebug("Not applying rules, because skip flag was set.");
                return;
            }

            if (!female) {
                Logger.LogWarning("Attempting to apply the wear rule, but the passed Female arg is null.");
                return;
            }
            ...
            foreach (var ruleSet in ruleset) {
                ...
                if (!IsCurrentlyOnTheChara(dataArrForWears, ruleSet.MasterItem) &&
                    ...
                    Logger.LogDebug($"This MasterItem isn't worn by the character <{female.heroineID}> - skip.");
                    continue;
                }
                ...
                foreach (var slaveItem in ruleSet.SlaveItems) {
                    ...
                    if (!IsCurrentlyOnTheChara(dataArrForWears, slaveItem) &&
                        ...
                        Logger.LogDebug($"This SlaveItem isn't worn by the character <{female.heroineID}> - skip.");
                        continue;
                    }
                    ...
                    switch (slaveItem.MasterItemState.Value) {
                        ...
                        default:
                            Logger.LogError("[ItemsCoordinator:ApplyRules] Unknown type of MasterItem!");
                            return;
                    }
                }
            }
        ...
        internal static void ApplyRuleset(
            ...
            if (!Enum.IsDefined(typeof(WEAR_SHOW_TYPE), itemPart)) {
                Logger.LogWarning("Attempting to apply the wear rule, but the passed Master item data is illegal.");
                return;
            }
        ...
        internal static void ApplyRuleset(
            ...
            if (female == null) {
                Logger.LogWarning("Attempting to apply the accessory rule, but the passed Female arg is null.");
                return;
            }
        ...
        private static void TryToApplyRule(InterItemRuleSet interItemRuleSet, Female female, OneOf<WEAR_SHOW, bool> masterItemState, SlaveItem slaveItem) {
            if (interItemRuleSet.MasterItem.ItemData.IsT0 && !masterItemState.IsT0 || interItemRuleSet.MasterItem.ItemData.IsT1 && !masterItemState.IsT1){
                Logger.LogWarning("Inconsistent data types between RuleSet.MasterItem.ItemData " +
            ...
            if (masterItemState.IsT0) {
                ...
                Logger.LogWarning("Unknown type of MasterItemState!");
                return;
            }
            ...
            switch (slaveItem.ItemData.Value) {
                case DataOfWearItem slaveItemDataCastToWear:
                {
                    if (!slaveItem.SlaveItemState.IsT0) {
                        Logger.LogWarning("Inconsistent data types between RuleSet.SlaveItem.ItemData " +
                ...
                default:
                    Logger.LogError("Unknown type of SlaveItem!");
                    return;
            }
        }
        ...
        private static void PrintRuleInfo(
            ...
            Logger.LogDebug("------------------------------------------");
            Logger.LogDebug(female.heroineID.ToString());

            switch (dataOfMasterItem.Value) {
                case DataOfWearItem masterCastToWear:
                    Logger.LogDebug($"Master {masterCastToWear.ItemPart} is {masterItemState.AsT0}");
                    break;
                case DataOfAccessoryItem masterCastToAccessory:
                    Logger.LogDebug($"Master {masterCastToAccessory.SlotNo} is {masterItemState.AsT1}");
                    break;
                default:
                    Logger.LogError("[ItemsCoordinator:PrintRuleInfo] Unknown type of MasterItem!");
                    return;
            }

            switch (dataOfSlaveItem.Value) {
                case DataOfWearItem slaveCastToWear:
                    Logger.LogDebug($"Setting {slaveCastToWear.ItemPart} to {slaveItemState.AsT0}");
                    break;
                case DataOfAccessoryItem slaveCastToAccessory:
                    Logger.LogDebug($"Setting {slaveCastToAccessory.SlotNo} to {slaveItemState.AsT1}");
                    break;
                default:
                    Logger.LogError("Unknown type of SlaveItem!");
                    return;
