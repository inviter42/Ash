...
using BetterTattoosStudio.Utils;
using HarmonyLib;
using UnityEngine;
using Object = UnityEngine.Object;

namespace BetterTattoosStudio.Hooks._Head
{
    internal class HeadHooks
    {
        ...
        internal static bool RendSkinTexture_FemalePrefix(Head __instance) {
            ...
            var tattooDataArray = TattooDataManager.TattooDataDict.GetValueOrDefaultValue(part, null);

            if (tattooDataArray?.Count(data => data != null) > 1) {
                BetterTattoosStudio.Logger.LogDebug("Multi-texture tattoo rendering branch");

                mat.SetTexture(TattooTex, TattooTextureUtils.StackTattoosAndWriteOutput(__instance, part));
                mat.SetColor(TattooColor, Color.white);
            }
            else {
                BetterTattoosStudio.Logger.LogDebug("Default tattoo rendering branch");
